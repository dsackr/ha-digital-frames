...
# Cache-path suffix for bins generated by the opt-in "vivid" color pipeline
# (CONF_COLOR_PIPELINE). Keeps them out from under the fast-pipeline
# directory so switching the pipeline never serves stale bytes from the
# other one -- the *wire* codec_id passed to encode_for_panel is untouched
# (byte layout doesn't change, only the color quantization does).
_VIVID_CACHE_SUFFIX = "_vivid"
...
def _cache_codec_id(codec_id: str, color_pipeline: str) -> str:
    """Cache-path codec id for *codec_id* under *color_pipeline*.

    Only Spectra codecs (color_mode == "spectra6") dither at all, so JPEG/
    PNG codec ids pass through unchanged regardless of *color_pipeline*.

    The fast-pipeline path is always tagged with
    image_converter.FAST_PIPELINE_CACHE_VERSION, so a future change to what
    "fast" produces (like 2026-08-09's enhance-chain addition) can bump that
    version and old cached bins simply miss and regenerate, instead of
    silently serving stale colors until something else invalidates them.
    """
    from .panel_codec import CODECS  # noqa: PLC0415

    codec = CODECS.get(codec_id)
    if codec is None or codec.color_mode != "spectra6":
        return codec_id
    if color_pipeline == COLOR_PIPELINE_VIVID:
        return f"{codec_id}{_VIVID_CACHE_SUFFIX}"
    from .image_converter import FAST_PIPELINE_CACHE_VERSION  # noqa: PLC0415

    return f"{codec_id}_v{FAST_PIPELINE_CACHE_VERSION}"


def _all_render_targets(
    hass: "HomeAssistant",
) -> list[tuple[RenderSpec, str, str]]:
    """(RenderSpec, wire codec_id, color_pipeline) triples for every
    configured frame.

    Distinct by (spec, cache codec id) so Meural JPEG and Spectra at the
    same geometry -- or the same Spectra panel under different color
    pipelines -- never share a cache slot incorrectly.
    """
    from .panel_codec import panel_codec_for_entry  # noqa: PLC0415

    seen: set[tuple[RenderSpec, str]] = set()
    out: list[tuple[RenderSpec, str, str]] = []
    for entry in hass.config_entries.async_entries(DOMAIN):
        if entry.data.get("kind") == "scenes_hub":
            continue
        width = entry.data.get(CONF_WIDTH)
        ...
        try:
            ...
            continue
        color_pipeline = _color_pipeline_for_entry(entry)
        cache_codec_id = _cache_codec_id(codec_id, color_pipeline)
        spec = render_spec_for_hass_entry(hass, entry)
        key = (spec, cache_codec_id)
        if key in seen:
            continue
        seen.add(key)
        out.append((spec, codec_id, color_pipeline))
    return out
...
def _known_codec_ids() -> tuple[str, ...]:
    """All codec ids (cache-path form) that may appear under bin/ (FramePort
    Phase 2): the bare codec id (pre-color-pipeline-versioning bins, kept so
    full-invalidation loops can still find and clean up old ones), plus the
    current fast-pipeline version and the vivid-pipeline variant of each
    Spectra codec -- callers use this to enumerate/invalidate every cached
    render of an image across codecs, so a bin left behind by a since-
    reverted CONF_COLOR_PIPELINE choice or a past pipeline version must be
    reachable too."""
    from .panel_codec import CODECS  # noqa: PLC0415
    ...
    for codec_id, codec in CODECS.items():
        ids.append(codec_id)
        if codec.color_mode == "spectra6":
            ids.append(_cache_codec_id(codec_id, COLOR_PIPELINE_FAST))
            ids.append(_cache_codec_id(codec_id, COLOR_PIPELINE_VIVID))
    return tuple(ids)
...
class LibraryManager:
    ...
    async def _backfill_one(self, record: LibraryImage) -> None:
        ...
        missing = [
            (spec, codec_id, color_pipeline)
            for spec, codec_id, color_pipeline in _all_render_targets(self.hass)
        ...
        for spec, codec_id, color_pipeline in missing:
            try:
            ...
            await self._backend.async_save_bin(
                ...
                bin_bytes,
                spec.variant,
                _cache_codec_id(codec_id, color_pipeline),
            )
        ...
        *,
        ...
        if pack_method is None and crop_box_override is None:
            cached = await self._backend.async_get_bin(
                image_id, width, height, spec.variant, cache_codec_id
            )
            if cached is not None:
                return cached
        ...
        if pack_method is None and crop_box_override is None:
            await self._backend.async_save_bin(
                image_id, width, height, bin_bytes, spec.variant, cache_codec_id
            )
        return bin_bytes
