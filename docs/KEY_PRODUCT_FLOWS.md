...
Lives natively on the Walls tab (KPF 19) — not a separate tab, not a separate "wallpaper wall." A "🖼 Wallpaper" button next to Align Wall to Grid toggles wallpaper-editing view for the currently active wall; a "🖼 Choose Image…" button (visible only in that view) opens a library-image picker.
...
- **Entry points**: `wall_geometry.py` (`compute_wallpaper_crop_boxes`), `walls_http.py` (`DigitalFramesWallWallpaperView`), `scenes.py` (`Scene.wallpaper`, `_validate_wallpaper`, `async_send_mappings`), `panel_codec.py` (`encode_for_panel`/`encode_for_panel_with_preview`'s `dest_box`, `_compose_rgb`), `image_converter.py` (`_process_cropped`'s windowed-placement branch, `_dest_pixel_box`, `_paste_windowed`), `library.py` (`async_get_bin_for_send`'s `dest_box_override`), `__init__.py` (view registration),
  `digital-frames-panel.js` (`_toggleWallWallpaperMode`, `_loadWallWallpaperFromScene`, `_renderWallWallpaperTiles`, `_wallpaperImageBeginDrag`/`_onWallpaperImagePointerMove`/`_onWallpaperImagePointerUp`, `_selectWallWallpaperImage`, `_openWallWallpaperPicker`, `_saveWallWallpaper`, `_wallContentBoundingBox`, `_wallActiveWallpaper`).
- **If it silently breaks**: dragging a frame moves/resizes/distorts the background image (the original bug above, regressed), a frame's crop is wrong or a frame with no overlap gets sent a degenerate crop, a partially-overlapping frame shows the covered slice stretched to fill the whole frame instead of leaving the rest black (the `dest_box` bug below, regressed), re-saving a loaded wallpaper scene creates a duplicate instead of updating it (or errors with "a scene named ... already exists"), reopening a saved wallpaper scene doesn't restore its pan/zoom, or Send to Frames Now 500s.
