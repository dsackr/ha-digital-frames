...
**A wallpaper is not a separate entity — it's just a Scene.** Saving one creates/updates an ordinary Scene whose mappings are `image_crop` entries (image_id + crop_box) all sharing one background image_id, plus an optional `wallpaper` field on the Scene recording that image's rect (`{image_id, x, y, width, height}` in wall-canvas px) purely so the editor can restore the exact pan/zoom when reopened — like `Scene.album`, it's never consulted when the scene is actually sent (KPF 16). This resolves cleanly with the rest of the data model: the wall (KPF 19) only ever stores frame positions, never images, so it was never a candidate for holding this state either.

**The image's rect is completely independent of frame placement** — this is the core property the feature exists to guarantee, after an earlier version got it backwards (see the bug note below). Frames are transparent windows (`.wall-wallpaper-tile`, replacing the normal thumbnail skin the instant an image is chosen) dragged with the exact same machinery the Walls tab always used (`_wallBeginDrag`, `_onWallPointerUp`, `_scheduleWallLayoutSave`, unmodified) — moving a frame only changes which part of the image's fixed rect that frame's window reveals. Panning the image (drag its body) and zooming it (drag its blue corner handle, uniform scale anchored at the top-left, aspect ratio always locked to the image's own) are a separate pointer-drag interaction on the image layer itself (`_wallpaperImageBeginDrag`/`_onWallpaperImagePointerMove`) that never touches frame positions. A frame's crop is computed as the fractional intersection of its own wall-canvas rect with the image's rect (`wall_geometry.compute_wallpaper_crop_boxes`) — a frame with no overlap at all gets excluded from the scene rather than sent a degenerate crop.

**A frame that only partly overlaps the image shows exactly that part, and leaves the rest of the frame black** — it is never stretched to fill the frame (a real bug, see below). `compute_wallpaper_crop_boxes` returns two things per frame: `crop_box` (which pixels of the *image* are behind this frame, as a fraction of the image) and `dest_box` (where within *this frame's own canvas* that content belongs, as a fraction of the frame — `(0,0,1,1)` when the frame is fully covered, a sub-rectangle otherwise). `panel_codec.encode_for_panel`'s `dest_box` param places the crop only within that sub-rectangle of the final wire image and fills the remainder with black, for both the Spectra path (`image_converter._process_cropped`'s windowed-placement branch) and the JPEG/PNG path (`panel_codec._compose_rgb`); a `dest_box` of `None` or full coverage reproduces the exact old crop-and-fill behavior byte-for-byte, so every pre-existing crop_box caller (wall-banner messages, the per-frame Adjust Crop editor, both of which always intend full coverage) is unaffected. `dest_box` rides along in the saved scene's `image_crop` mapping (optional; a mapping saved before this field existed defaults to full coverage) so a wallpaper scene activated later (KPF 16) composites identically to an immediate "Send to Frames Now".

The Walls tab's own in-browser preview reaches this same picture for free, without any of the above: the background is one absolutely-positioned `<div>` sized to the image's rect, and every frame's window just sits on top of it at its own wall-canvas position with a transparent background — wherever a frame's window doesn't overlap the div, the canvas's own background (not the image) shows through, which already reads as "blank" without any explicit crop math on the client. Getting the physical frame's rendering to match what the browser already showed is exactly what `dest_box` is for.

"Save as Scene" and "Send to Frames Now" (the Walls tab's existing scene-save/send buttons, dispatching to wallpaper-specific logic while wallpaper mode is active) both post to `POST /api/digital_frames/walls/{wall_id}/wallpaper` with the image_id and current image_rect, differing only in `push_now` (default `true`; `false` for Save as Scene so building/previewing never touches hardware). Both first flush any in-flight debounced drag save (`_flushWallLayoutSave`) so the backend computes crops against the wall's latest frame positions, not a stale record. Editing a previously-saved wallpaper scene (selected from the Walls tab's own scene `<select>` — the same picker, since a wallpaper really is just a scene) restores its stored rect (`_loadWallWallpaperFromScene`) and threads its `scene_id` back on save so it updates in place instead of colliding with its own name as a "new" scene; picking a scene with no `wallpaper` field is an ordinary per-frame scene preview, unaffected.

Until an image is chosen, a frame's window has nothing to show through yet, so it shows what's actually on that physical frame right now (its own live thumbnail via `/api/digital_frames/frame/{entry_id}/thumbnail`) instead of an empty box — the instant an image is picked, every tile drops its thumbnail for the transparent skin. This same "transparent window over one shared background div" skin is also used to preview a loaded wallpaper scene on the *ordinary* Walls tab (`_wallActiveWallpaper`), not just inside wallpaper-editing mode — see the bug note below.

**Real bug this replaced (image resize-with-frame-drag):** an earlier version derived the image's on-screen box from the *bounding box of wherever the frames currently were*, non-uniformly stretching the image to fill it (`object-fit: fill`) — so dragging a frame grew/reshaped the bounding box, which grew/distorted the "image" as a side effect, and picking an image with a different aspect ratio than the frame layout stretched it unevenly. Reported directly by a user: *"zooming in on the wall...shouldn't change the image aspect ratio (it does today)."* Fixed by making the image's rect independent, explicit, user-controlled state (this KPF) rather than something derived from frame geometry every render.
- **Entry points**: `wall_geometry.py` (`compute_wallpaper_crop_boxes`), `walls_http.py` (`DigitalFramesWallWallpaperView`, `DigitalFramesWallSendView`), `scenes.py` (`Scene.wallpaper`, `_validate_wallpaper`, `async_send_mappings`), `panel_codec.py` (`encode_for_panel`/`encode_for_panel_with_preview`'s `dest_box`, `_compose_rgb`), `image_converter.py` (`_process_cropped`'s windowed-placement branch, `_dest_pixel_box`, `_paste_windowed`), `library.py` (`async_get_bin_for_send`'s `dest_box_override`), `__init__.py` (view registration),
  `digital-frames-panel.js` (`_toggleWallWallpaperMode`, `_loadWallWallpaperFromScene`, `_renderWallWallpaperTiles`, `_wallpaperImageBeginDrag`/`_onWallpaperImagePointerMove`/`_onWallpaperImagePointerUp`, `_selectWallWallpaperImage`, `_openWallWallpaperPicker`/`_loadWallWallpaperPickerImages`, `_saveWallWallpaper`, `_wallContentBoundingBox`, `_wallActiveWallpaper`, `_sendWallToFrames`'s `cropTargets`/`_sendCropMappingsToFrames`, `_loadFrameThumbnail`).
- **If it silently breaks**: dragging a frame moves/resizes/distorts the background image (the original bug above, regressed), a frame's crop is wrong or a frame with no overlap gets sent a degenerate crop, a partially-overlapping frame shows the covered slice stretched to fill the whole frame instead of leaving the rest black (the `dest_box` bug below, regressed), re-saving a loaded wallpaper scene creates a duplicate instead of updating it (or errors with "a scene named ... already exists"), reopening a saved wallpaper scene doesn't restore its pan/zoom, the image picker's Album filter is missing or does nothing, its thumbnails look cropped to an arbitrary corner instead of a centered square (the picker-markup bug below, regressed), the ordinary Walls-tab "Send to Frames" button silently does nothing for wallpaper-mapped frames (the sixth bug below, regressed), a frame's "on frame"/"on deck" tile shows a broken-image icon instead of its actual thumbnail (the seventh bug below, regressed), or Send to Frames Now 500s.
  A real bug fixed after a live user report on the (now-removed) prior implementation of this feature: the per-frame send loop called `coordinator.async_send_image_or_queue(wire_bytes=..., preview_png=...)`, but every real coordinator (`DigitalFramesCoordinator`/`MeuralCoordinator`/`SamsungCoordinator`) takes a positional `image_bytes` and a `thumbnail` keyword — an immediate, unconditional `TypeError` for every frame, on every wall, confirmed via direct API calls against a production instance (`push_now=false` succeeded; `push_now=true` 500'd). The test suite's own coordinator fake was mocked with the same wrong keyword names as the bug, so it never caught this; fixed to match the real signature, and the fake corrected to match reality. The send loop is also hardened so one frame's encode/send failure doesn't abort every other frame's delivery, and the response reports per-frame `results`/`frames_failed` instead of a blanket "success".
  A second, related bug reported right after the first was fixed: every frame's Walls-tab tile thumbnail showed the identical, uncropped background image. `async_send_image_or_queue` was called with *both* `image_id=<shared background>` and `thumbnail=<this frame's correct crop>` — but `coordinator.async_set_last_image`'s own contract is to pass exactly one, and the Walls tab's tile-render priority checks `image_id` before `thumbnail`, so the correct per-frame preview was silently ignored. Fixed by omitting `image_id` for crop-based sends, both here and in `scenes.py`'s `async_send_mappings` (KPF 16) — the latter also never generated a per-frame preview at all before this fix, always passing `thumbnail=None`; a follow-up fix made this omission unconditional (not just on preview-generation success), since a preview failure was silently falling back to reporting the shared `image_id` again — quietly reintroducing the same bug on that one error path.
  A third bug, reported the same day: outside of wallpaper-*editing* mode, every frame's ordinary Walls-tab tile showed the *same, full, uncropped* background image (there was "no crop-rendering endpoint" to ask for anything else) instead of its own slice — so a saved wallpaper scene was effectively impossible to visually distinguish frame-by-frame without opening the editor. Fixed by extending the editor's own "transparent window over one shared background div" trick to the ordinary tile-preview path too (`_wallActiveWallpaper`), which needed no new endpoint since the browser already has the full image and only needs the same positioning math.
  A fourth bug, reported alongside the third: a frame that only *partly* overlapped the wallpaper's image rect showed the correct black/blank area in the browser preview, but the physical frame received the partial crop stretched to fill the entire frame instead — because `_compose_rgb`/`_process_cropped` had always assumed a crop_box fully covers its target canvas (true for every prior crop_box caller — wall-banner messages, the per-frame Adjust Crop editor — but not for a wallpaper frame hanging off the image's edge). Fixed by adding the `dest_box` windowed-placement path described above.
  A fifth bug, reported after the first four fixes shipped: the "Choose Image…" picker had no Album filter (it always listed every library image, unlike every other picker in the app), and every thumbnail looked cropped to an arbitrary corner rather than a clean centered square. Root cause: its grid cells were a one-off plain `<div>` with no sizing/cropping rule for the `<img>` `_loadThumbnail` injects, unlike the per-frame picker's `.image-picker-thumb img { object-fit: cover }` rule — so the fetched (up to 480px) thumbnail rendered at its natural size and the cell's `overflow: hidden` just clipped whatever corner happened to land in the top-left, not a centered crop. Fixed by adding the same Album `<select>` the per-frame picker has and reusing its `.image-picker-cell`/`.image-picker-thumb` markup outright instead of a bespoke one.
  A sixth bug, reported after the first five fixes shipped and initially believed (wrongly) to be the root cause of a repeated "thumbnails aren't saved after a wallpaper is sent" report: the *ordinary* (non-wallpaper-editing) Walls-tab "Send to Frames" button builds its send list by filtering `_wallEffectiveMapping` down to `typeof mapping === 'string'` (a plain image_id) or `mapping.type === 'skill'` -- an `image_crop` mapping is neither, so a wallpaper-mapped frame matched *nothing* and was silently dropped from the send entirely, with the button still reporting "✓ Sent to N frames" for whichever plain-image/skill frames *did* match. This only reproduced when a saved wallpaper scene was loaded and sent from the *ordinary* Walls tab (not the wallpaper editor's own "Send to Frames Now", which always worked). Confirmed live: `POST .../wallpaper` with `push_now=true` demonstrably updates each frame's thumbnail (verified by hashing `/frame/{id}/thumbnail` before and after against a production wall), proving the compose/send pipeline itself was sound -- the bug was entirely in which frames the ordinary send button even attempted. Fixed by adding a `cropTargets` category alongside `imageTargets`/`skillTargets` in `_sendWallToFrames`, sent as one batch via the new `DigitalFramesWallSendView` (`POST /api/digital_frames/walls/{wall_id}/send`, body `{"mappings": {entry_id: mapping}}`) to `SceneManager.async_send_mappings` -- the same generic mapping-batch executor schedules, skills, and message sends already used, which already handled `image_crop` mappings correctly. A related correctness bug caught in the same pass: `DigitalFramesWallWallpaperView`'s own send loop set `sent = True` unconditionally after its coordinator-lookup block, so a frame with no coordinator registered was both silently skipped *and* reported as a success -- fixed to report `sent = False` with a message in that case.
  A seventh bug, reported immediately after the sixth fix shipped ("it's not working") -- and the *actual*, final root cause across every one of these reports, hiding underneath all six fixes above the whole time: three places rendered a frame's "on frame"/"on deck" tile -- the *only* thing left on-screen once a send actually completes, no matter which of the six previous bugs was also in play that day -- via a plain `<img src="/api/digital_frames/frame/{entry_id}/thumbnail">`. That endpoint `requires_auth = True` like every other `/api/` route, and HA's frontend authenticates with a Bearer token attached by JavaScript on each `fetch()` call -- an `<img>` tag has no way to attach a header, so the browser's request always ran unauthenticated and the endpoint 401'd, in every real Home Assistant session, for as long as this code existed. Direct `curl` calls with a manually-supplied token (used throughout this investigation to verify the backend) never touched this path and so never surfaced it; the Playwright mock server didn't reject unauthenticated requests either, so none of the extensive test coverage added for bugs one through six caught it. First caught by actually driving the real browser UI against production and reading the Network panel: every `/frame/.../thumbnail` request came back 401, and every affected tile showed a broken-image icon -- confirmed visually before and after the fix. The pattern this endpoint needed -- authenticated `fetch()` with the Bearer header, response blob turned into an object URL, `<img src>` set to *that* -- already existed for every other thumbnail in the panel (`_loadThumbnail`) and in the separate `digital-frames-card.js` Lovelace card (which was never affected, since it already fetched this same endpoint correctly); it just hadn't been applied to these three call sites. Fixed by adding `_loadFrameThumbnail(entryId, container)`, the same authenticated-fetch-to-blob-URL pattern under a dedicated cache-key prefix (`frame:{entry_id}`, distinct from `_loadThumbnail`'s library-image_id keys), and switching all three sites to it.
- **Test status**: **Backend-tested** — `tests/python/managers/test_wall_geometry.py` (`compute_wallpaper_crop_boxes`: a frame's crop depends only on its own position and the image rect, proven by moving one frame and checking another's crop is unchanged; a fully-covered frame gets `dest_box == (0,0,1,1)`; a partially-overlapping frame gets a narrower `dest_box`; partial-overlap `crop_box` clamping; no-overlap → `None`; invalid image_rect rejected);
  `tests/python/managers/test_wall_wallpaper.py` (`DigitalFramesWallWallpaperView`: a fake coordinator matching every real driver's actual `async_send_image_or_queue` signature — which reproduces the exact live `TypeError`/500 against unpatched code — omitting `image_id` while still passing a non-null `thumbnail`; moving one frame leaves another frame's saved crop_box unchanged; a frame outside the image rect excluded from the saved scene; a partially-overlapping frame's saved mapping has a non-full `dest_box`; re-saving by `scene_id` updates in place rather than duplicating; a name collision with no `scene_id` surfaces `scene_save_error` instead of crashing; one frame's send failure doesn't block another's; a frame with no coordinator registered is reported as a failure, not a false success);
  `tests/python/managers/test_wall_send.py` (`DigitalFramesWallSendView`: 404 for an unknown wall, 400 for a missing/empty `mappings` body; an `image_crop` mapping actually reaches the frame's coordinator with `image_id` omitted and a non-null per-frame thumbnail -- the bug this view exists to fix; a batch mixing a plain image_id and an `image_crop` mapping both succeed in one call; a coordinator failure is reported per-frame, not masked as a blanket success);
  `tests/python/managers/test_scenes.py` (`dest_box` on an `image_crop` mapping reaches both `library_manager.async_get_bin_for_send`'s `dest_box_override` and the preview compose call; a preview-generation failure still omits `image_id` rather than falling back to the shared one);
  `tests/python/unit/test_panel_codec.py` (`dest_box=None`/full-coverage byte-identical to omitting it entirely, for both Spectra and JPEG; a partial `dest_box` leaves the uncovered half of the composed preview dark on both codec paths, not a stretched copy of the covered half).
  **Panel-tested** — `tests/panel/walls-wallpaper.spec.js` (lives on the Walls tab, no separate tab/picker; frames show their own live thumbnail until an image is chosen, then drop it for a transparent window; dragging a frame never changes the background rect; panning the image never moves any frame; the corner handle zooms uniformly with aspect ratio preserved and position anchored; Save as Scene posts `push_now:false` vs Send to Frames Now posts `push_now:true`; re-saving threads `scene_id` back; selecting an existing wallpaper scene restores its stored rect; previewing a wallpaper scene on the *ordinary* Walls tab — not the editor — shows one shared background behind transparent frame windows, each still keeping its normal footer/hover affordances, instead of a full image loaded per tile; the image picker's Album filter lists every album and narrows the grid when changed, and its cells use the same `.image-picker-cell`/`.image-picker-thumb` markup as the per-frame picker; clicking the *ordinary* Walls-tab "Send to Frames" button after loading a wallpaper scene — not the editor's own send — actually posts that scene's `image_crop` mappings to `DigitalFramesWallSendView` instead of silently dropping them; a frame's "on frame" thumbnail request — the mock server tracks this specifically, since the mock never rejects an unauthenticated request the way real HA does — carries the `Authorization` header, not a bare unauthenticated `<img src>`).
...
## 38. Art Factory (RETIRED — AI generation removed; wallpaper mode moved to the Walls tab)
**Retired.** The "🎨 Art Factory" tab, its standalone AI-generation feature (`art_factory_http.py`, `async_generate_ai_art_image`), and its bolted-on wall-spanning UI are **deleted**. Generation never reliably produced an image matching the prompt — every real path (HA `ai_task`, the Pollinations.ai fallback, and the last-resort PIL fallback) could bottom out at a canvas that just draws the prompt's *text* onto it, which is what a live user was actually seeing: "the image gets created but always is an image of the words in the prompt not the image those words described." Rather than ship a fix for a feature layered on top of that broken foundation, the whole tab was dropped to be revisited later.
The wallpaper-spanning half of Art Factory (arranging one image across a wall's frames) was **not** dropped — it was moved to live natively on the Walls tab, since a wallpaper is just a Scene and belongs wherever scenes are already edited. See **KPF 36** for the current implementation.
- **Entry points**: none (removed).
- **Test status**: N/A (removed). `tests/python/managers/test_art_factory.py` and `tests/panel/art-factory.spec.js` were deleted with it.
