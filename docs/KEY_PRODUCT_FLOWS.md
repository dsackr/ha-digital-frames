...
- **If it silently breaks**: dragging a frame moves/resizes/distorts the background image (the original bug above, regressed), a frame's crop is wrong or a frame with no overlap gets sent a degenerate crop, a partially-overlapping frame shows the covered slice stretched to fill the whole frame instead of leaving the rest black (the `dest_box` bug below, regressed), re-saving a loaded wallpaper scene creates a duplicate instead of updating it (or errors with "a scene named ... already exists"), reopening a saved wallpaper scene doesn't restore its pan/zoom, the image picker's Album filter is missing or does nothing, its thumbnails look cropped to an arbitrary corner instead of a centered square (the picker-markup bug below, regressed), the ordinary Walls-tab "Send to Frames" button silently does nothing for wallpaper-mapped frames (the sixth bug below, regressed), or Send to Frames Now 500s.
  ...
  A sixth bug, and the actual root cause behind three separate "thumbnails aren't saved after a wallpaper is sent" reports that the first five fixes above never touched: the *ordinary* (non-wallpaper-editing) Walls-tab "Send to Frames" button builds its send list by filtering `_wallEffectiveMapping` down to `typeof mapping === 'string'` (a plain image_id) or `mapping.type === 'skill'` -- an `image_crop` mapping is neither, so a wallpaper-mapped frame matched *nothing* and was silently dropped from the send entirely, with the button still reporting "✓ Sent to N frames" for whichever plain-image/skill frames *did* match. This only reproduced when a saved wallpaper scene was loaded and sent from the *ordinary* Walls tab (not the wallpaper editor's own "Send to Frames Now", which always worked -- see `DigitalFramesWallWallpaperView` above) -- e.g. reopening a saved wallpaper scene later, rather than sending immediately while still in the editor. Confirmed live: `POST .../wallpaper` with `push_now=true` demonstrably updates each frame's thumbnail (verified by hashing `/frame/{id}/thumbnail` before and after against a production wall), proving the compose/send pipeline itself was sound -- the bug was entirely in which frames the ordinary send button even attempted. Fixed by adding a `cropTargets` category alongside `imageTargets`/`skillTargets` in `_sendWallToFrames`, sent as one batch via the new `DigitalFramesWallSendView` (`POST /api/digital_frames/walls/{wall_id}/send`, body `{"mappings": {entry_id: mapping}}`) to `SceneManager.async_send_mappings` -- the same generic mapping-batch executor schedules, skills, and message sends already used, which already handled `image_crop` mappings correctly; the per-frame image-send endpoint (`/api/digital_frames/library/send`, which `imageTargets` still uses) has no way to express a crop override at all, so a batched call was required rather than another one-off per-frame endpoint. A related correctness bug caught in the same pass: `DigitalFramesWallWallpaperView`'s own send loop set `sent = True` unconditionally after its coordinator-lookup block, so a frame with no coordinator registered (not yet loaded, a stale entry_id) was both silently skipped *and* reported as a success -- fixed to report `sent = False` with a message in that case.
