...
- **`meural.py` / `meural_coordinator.py`** — Meural driver: LAN identify + postcard JPEG; when a Netgear account is linked (`meural_cloud_store`), sends also pin to Meural cloud (single-image gallery, replace prior item). Config entries use `driver=meural`.
- **`meural_cloud.py` / `meural_cloud_store.py`** — optional Meural cloud client (auth, upload, galleries, device load) and shared credentials + pin/album bindings.
